...
{% if execute %}
    ...
    {% if var('is_delta_load') and var('batch_name') != 'NA' %}
        ...
        {% if var('delta_load_batch_id') == -1 %}
            {% do run_query("insert into delta_load_job_control (batch_seq_id, job_table_name, job_table_status, table_load_start_time) values (" ~ run_started_at.strftime("%s") ~ ", '" ~ table_name ~ "', '" ~ var('started_status') ~ "', current_timestamp)") %}
        {% else %}
            ...
            {% if macro_run_snapshot_yn(table_name) %}
                {% do run_query("insert into delta_load_job_control (batch_seq_id, job_table_name, job_table_status, table_load_start_time) values (" ~ var('delta_load_batch_id') ~ ", '" ~ table_name ~ "', '" ~ var('started_status') ~ "', current_timestamp)") %}
            {% endif %}            
        {% endif %}
    {% else %}

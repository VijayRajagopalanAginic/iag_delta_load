    ...
    {% if var('is_delta_load') and var('batch_name') != 'NA' %}
        ...
        {% for res in results -%}
            {% if macro_run_snapshot_yn(res.node.name) %}
                {% do run_query("insert into delta_load_job_control (batch_seq_id, job_table_name, job_table_status, table_load_start_time) values (" ~ batch_id ~ ", '" ~ res.node.name ~ "', '" ~ res.status ~ "', current_timestamp)") %}
            {% endif %}
        ...
        {% set jobFailed = run_query("select count(*) from delta_load_job_control where batch_seq_id = " ~ batch_id ~ " and job_table_status = '" ~ var('error_status') ~ "'") %}

        {% if jobFailed|length > 0 and jobFailed[0][0] > 0 %}
            {% do run_query("insert into delta_load_master_control (sequence_id, batch_name, status, started_time) values (" ~ batch_id ~ ", '" ~ var('batch_name') ~ "', '" ~ var('error_status') ~ "', current_timestamp)") %}
        {% else %}
            {% do run_query("insert into delta_load_master_control (sequence_id, batch_name, status, started_time) values (" ~ batch_id ~ ", '" ~ var('batch_name') ~ "', '" ~ var('success_status') ~ "', current_timestamp)") %}
        {% endif %}
    {% else %}

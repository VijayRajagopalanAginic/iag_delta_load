...
{% if execute %}
    {% if var('delta_load_batch_id') != -1 %}
        {% set hasJobRunBefore = run_query("select count(*) from delta_load_job_control where batch_seq_id = " ~ var('delta_load_batch_id') ~ " and job_table_name = '" ~ table_name ~ "' and job_table_status = '" ~ var('success_status') ~ "' ") %}
